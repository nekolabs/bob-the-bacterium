-- turret.lua Type III - The Turret

local lg = love.graphics
local pathToEnemSrc = 'lua/enemies/' 

require(pathToEnemSrc .. 'turretShot')

local l21 = love.graphics.newImage("assets/enemy/turret.png") -- turret img, model L21, well and the only one
local l21Height, l21Width = l21:getHeight(), l21:getWidth()

turret = {} -- the table which holds the important parameters concerning the turret

local shotTimes = 0 -- number of times you hurt The Turret (max is 2)
local countFrom, countSpeed, countAim = 0,1,1 -- Timers for shooting rate of the turret

function createTurret(x, y) -- turret not moving so neither vx nor vy 
...
function turret_draw()
	for ei,ev in ipairs(turret) do  
		love.graphics.draw(l21, ev.x, ev.y, rot,scale,scale)
	end
end
