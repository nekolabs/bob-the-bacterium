...
local bulletImg = lg.newImage('assets/bullet.png')
--local atlMap = atl.Loader.load('level0.tmx') 
local buW, buH = bulletImg:getWidth(), bulletImg:getHeight()
local gunshot, isVolume = la.newSource("sfx/gunshot.ogg"), .475 -- gun sound
gunshot:setVolume(isVolume)
...
function bullet_update(dt)
  for bi,bv in ipairs(bullet) do
    if bv.bulletDir == "clockwise" then  
     ...
     -- there and shoots around him like a gunslinger it wont use to cap. Bob shouldn't be fast enough to overtake
     -- the bullet
